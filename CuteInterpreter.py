...
#( define othern ( lambda ( x nn ) ( define n nn ) ( plusn ( x ) ) ) )
    ...
    def run_arith(self, arith_node):
        ...
        if arith_node.type is TokenType.MINUS:
            return Node(TokenType.INT, int(rhs1.value) - int(rhs2.value))
        if arith_node.type is TokenType.DIV:
            return Node(TokenType.INT, int(rhs1.value) / int(rhs2.value))
        if arith_node.type is TokenType.TIMES:
            return Node(TokenType.INT, int(rhs1.value) * int(rhs2.value))

    def run_relate(self, relate_node):
        rhs1 = relate_node.next
        rhs2 = rhs1.next if rhs1.next is not None else None
        rhs1 = self.run_expr(rhs1)
        rhs2 = self.run_expr(rhs2)
        if relate_node.type is TokenType.GT:
            if int(rhs1.value) > int(rhs2.value):
                return self.TRUE_NODE
        if relate_node.type is TokenType.LT:
            if int(rhs1.value) < int(rhs2.value):
                return self.TRUE_NODE
        if relate_node.type is TokenType.EQ:
            if int(rhs1.value) == int(rhs2.value):
                return self.TRUE_NODE
        else:
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            if not is_quote_list(rhs1):
                print("cdr error!")
            result = pop_node_from_quote_list(rhs1)
            result1 = create_quote_node(result.next, True)

            if create_quote_node(result.next, True).type is TokenType.LIST:
            ...
            if self.run_expr(rhs1.value).type is TokenType.TRUE:
                ...
                result = Node(TokenType.COND)
                result.next = rhs2
                return self.run_func(result)
        else:
            return None
    ...
    def run_expr(self, root_node):
        ...
        elif root_node.type is TokenType.QUOTE:
            return Node(TokenType.LIST,root_node)
        else:
    ...
    def run_list(self, l_node):
        ...
        if op_code.type in [TokenType.TIMES, TokenType.MINUS, TokenType.PLUS, TokenType.DIV]:
